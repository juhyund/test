...
public class Util {
	...
	/**
	 * Check Null String,If String is null, Return "".
	 * <p>
	 */
	...
	/**
	 * Check Null String,If String is null, Return true.
	 * <p>
	 * 
	 * @param str
	 * @return
	...
	/**
	 * Special character (delim).
	 * <p>
	 * same as StringTokenizer.
	 * <p>
	 */
	...
	/**
	 * Special character delete.. sooyee 11.14.
	 * <p>
	 */
	...
	public static void printByte(byte[] b) {
		try {
			for (int i = 0;; i++)
				logger.debug("[" + Integer.toHexString(b[i]) + "] ");
	...
	public static String frontAppendNStr(char append, String str, int length) {
		...
		try {
			...
			logger.error("Util.frontAppendNStr : " + e.getMessage());
		}
		return b.toString();
	}

	/**
	 * print StringBuffer to HexString.
	 * <p>
	 * 
	...
	public static void printHex(StringBuffer b) {
		try {
			for (int i = 0; i < b.length(); i++) {
				logger.debug(frontAppendNStr('0', Integer.toHexString(b.charAt(i)), 2) + " ");
				// if(((i+1)%24) == 0); // System.out.print("\n");
			}
			logger.debug("\n");

		} catch (Exception e) {
			logger.error("Util.printHex : " + e.getMessage());
		}

	}

	/**
	 * Transform to non empty String.
	 * <p>
	 * ex) 0x80 0x49 -->"8049"
	 * <p>
	 * 
	...
	public static String getNHexString(String b) {
		...
		try {
			...
			logger.error("Util.getNHexString() : " + e.getMessage());
		}
		return buf.toString();
	}

	/**
	 * Transform to with empty String.
	 * <p>
	 * For Logging
	 * <p>
	 * ex) 0x09 0x81 --> "09 81"
	...
	public static String getHexString(StringBuffer b) {
		...
		try {
			...
			logger.error("Util.getHexString : " + e.getMessage());
		}
		return buf.toString();
	}
	...
	/**
	 * Transform to with non empty String.
	 * <p>
	 * For Logging
	 * <p>
	 * ex) 0x09 0x81 --> "0981"
	...
	public static String currYymmdd1() {
		...
		try {
			...
			logger.error("Util.currYymmdd() : " + e.getMessage());
		}
		return dateString;
	}
	...
	public static String millisecond2Date(long atime) {
		...
		try {
			...
			logger.error("Util.millisecond2Date() : " + e.getMessage());
		}
		return dateString;
	}
	...
	public static String getDateDay(long atime) {
		...
		try {
			...
			logger.error("Util.getDateDay() : " + e.getMessage());
		}
	...
	public static String getDateMonth(long atime, int flag) {
		...
		try {
			...
			if (flag == 0) { /* prev month */
				if (month == 1) {
					date = new String(new Integer(year - 1).toString()) + new String(new Integer(12).toString());
				} else {
					date = new String(new Integer(year).toString()) + new String(new Integer(month - 1).toString());
				}
			} else {
				date = dateString;
			}
		} catch (Exception e) {
			logger.error("Util.getDateMonth() : " + e.getMessage());
		}
		...
		try {
			...
			if (flag == 0) { /* prev month */
				if (month == 1) {
					...
					if (month <= 10) {
						tmp = new String("0") + new String(new Integer(month - 1).toString());
					} else {
						tmp = new String(new Integer(month - 1).toString());
					}
					date = new String(new Integer(year).toString()) + tmp;
				}
			} else {
				date = dateString;
			}
	...
	/**
	 * ex) 2004-08-08 00:04 -> 2004-08-08 00:15
	 * <p>
	 * ex) 2004-08-08 00:00 -> 2004-08-08 00:00
	 * <p>
	 * ex) 2004-08-08 00:15 -> 2004-08-08 00:15
	 * <p>
	 * ex) 2004-08-08 00:16 -> 2004-08-08 00:30
	 * <p>
	 * 
	 * @param yymmddHHMM
	 * @return
	...
	/**
	 * ex) 2004-08-08 00:00 -> 2004-08-07 24:00
	 * <p>
	 * 
	 * @param yymmddHHMM
	 * @param min        (minutes)
	 ...
	 *
	 */
	public static String addMinYymmdd(String yymmddHHMM, int min) throws Exception {
		...
		try {
			 ...
			 * 
			 * yy = Integer.parseInt(dateString.substring(0,4)); mm =
			 * Integer.parseInt(dateString.substring(4,6)); day =
			 * Integer.parseInt(dateString.substring(6,8)); HH =
			 * Integer.parseInt(dateString.substring(8,10)); MM =
			 * Integer.parseInt(dateString.substring(10,12));
			 * 
			 * c.set(yy, mm-1, day-1, 0, 0,0); dateString = formatter.format(c.getTime());
			 * b.append(dateString.substring(0,8)); b.append("2400"); dateString =
			 * b.toString(); }
			 */
	 ...
	 *
	 */
	public static String addDateYyyymmdd(String yyyymmdd, int date) throws Exception {
	...
	public static String getYymmddhhmm(int yy, int mm, int dd, int hh, int min) throws Exception {
	...
	/**
	 * Get date from formatted "yyyyMMddHHmmss" Jeong Hun
	 *
