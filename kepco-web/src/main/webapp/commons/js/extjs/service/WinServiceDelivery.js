...
var winServiceDelivery = null;
//팝업 window
var winServiceDeliveryPop = function(service_no, delivery_no, cust_no, cust_nm, tel_mobile, zipcode, address1, address2, prd_nm, bad_cd1_nm, bad_cd2_nm, insert_dt) {
	
	serviceDeliveryform.getForm().reset();
	
	if(!winServiceDelivery) {
		
    	winServiceDelivery = Ext.create('Ext.window.Window', {
            title: '배송처리정보',
            height: 450,
            width: 600,
            closeAction: 'close',
	...
	if(winServiceDelivery.isVisible()) {
		winServiceDelivery.hide();
	} else {
		loadDataServiceDelivery(service_no, delivery_no, cust_no, cust_nm, tel_mobile, zipcode, address1, address2, prd_nm, bad_cd1_nm, bad_cd2_nm, insert_dt);
		winServiceDelivery.show();			
	}
}
 ...
 * @param service_no
 */
function loadDataServiceDelivery(service_no, delivery_no, cust_no, cust_nm, tel_mobile, zipcode, address1, address2, prd_nm, bad_cd1_nm, bad_cd2_nm, insert_dt) {

	var form = serviceDeliveryform.getForm();
	...
	if(delivery_no != undefined && delivery_no != "") {
		...
		if(form.findField("delivery_remark") == "") form.findField("delivery_remark").setValue(remark);
		...
		Ext.Ajax.request({
	        ...
	        success: function(response) {
	        	...
	        	form.findField("delivery_owner_nm").setValue(result.delivery_owner_nm);
	        	form.findField("tel_mobile").setValue(result.delivery_tel_mobile);
	        	form.findField("zipcode").setValue(result.delivery_zipcode);
	        	form.findField("address1").setValue(result.delivery_address1);
	        	form.findField("address2").setValue(result.delivery_address2);
	        	form.findField("remark").setValue(result.delivery_remark);
...
// 팝업 form
var serviceDeliveryform = Ext.create('Ext.form.Panel', {
	id: 'serviceDeliveryform',
    bodyPadding: 7,
    flex: 1,
    border: 0,
    layout: 'anchor',
    ...
    items: [
		...
		},
		{
	        ...
	        layout: 'hbox',         
	        defaultType: 'textfield',
	        items: [comboDeliveryStatus, comboDeliveryAgency]
		}, {
	        xtype: 'fieldcontainer',            
	        layout: 'hbox', 
	        defaultType: 'textfield',
    ...
    buttons: [
        ...
        handler: function() {
			...
			if(delivery_owner_nm.getValue() == "" ) {					
				focusMsgAlert('Warning', '수령자를 입력해 주시기 바랍니다.', delivery_owner_nm);
				return;
			}
			...
			if(delivery_status.getSubmitValue() == "" ) {					
				focusMsgAlert('Warning', '배송상태를 선택해 주시기 바랍니다.', delivery_status);
				return;
			}
			...
			if(delivery_status.getSubmitValue() == "DL002" ) {	// 발송완료				
