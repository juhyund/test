...
Ext.onReady(function() {
	...
	function onInsertPRD() {
		...
		for(var i = 0; i < selections.length; i++) {
			...
			var cost_amt = Number(currentRecord.get('as_price')) * Number(currentRecord.get('qty'));
			
			if(currentRecord.get('free_cost_yn') == "Y") cost_amt = 0; // 무상AS
			
			var record = {'prd_type' : currentRecord.get('prd_type')
				, 'free_cost_yn' : currentRecord.get('free_cost_yn')	
				, 'order_no': ''
				, 'prd_no': currentRecord.get('prd_no')
				, 'prd_nm': currentRecord.get('prd_nm')
				, 'prd_cate1': currentRecord.get('prd_cate1')
				, 'prd_cate1_nm': currentRecord.get('prd_cate1_nm')
				, 'service_no': ''
				, 'price': currentRecord.get('as_price')
				, 'qty': currentRecord.get('qty')
				, 'cost_amt': cost_amt
			};
