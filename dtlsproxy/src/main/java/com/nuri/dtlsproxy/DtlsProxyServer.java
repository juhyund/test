...
public class DtlsProxyServer {
	...
	@Value("${proxyserver.ipaddr}") 
	String localAddress;
	
	@Value("${proxyserver.listenport}") 
	String listenPort;
	
	@Value("${lwm2m.ipaddr}") 
	String relayServer;
	
	@Value("${lwm2m.listenport}") 
	String relayPort;
    ...
    public void start(String[] args)  {
        ...
        ret = server.bind(localAddress, listenPort);
        if (ret != 0) {
            System.err.println("server.bind() : " + Util.getErrorString(ret));
            return;
        }else {
        	System.out.println("DTLS+PSK Proxy Server : IP["+localAddress+"] PORT=[" +listenPort+"]");
        }
        ...
        while (true) {
            ...
            if(socket != null) {
               ...
               // ThreadServer localThreadServer = new ThreadServer(socket);
                ...
                ThreadServer localThreadServer = new ThreadServer(relayServer, relayPort, socket);
