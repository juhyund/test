...
int setConfig(struct instance *ins)
{
        char readBuff[512];
        char *temp;
		const char *conffile = NPROXY_CONFIG_FILE;
        ...
        if ( fp ){
                while ( !feof(fp) ) {
						...
						if ( strncasecmp(temp, "FEPServerIP", strlen("FEPServerIP")) == 0 ) {
									cfline = strrchr(temp, '=')+1;
									ins->fepserver_ip = (char*)malloc(strlen(cfline));
									memcpy(ins->fepserver_ip,cfline,strlen(cfline-1));
						}
						if ( strncasecmp(temp, "FEPServerPort", strlen("FEPServerPort")) == 0 ) {
									cfline = strrchr(temp, '=')+1;
									ins->fepserver_port = (char*)malloc(strlen(cfline));
									memcpy(ins->fepserver_port,cfline,strlen(cfline-1));
						}
                }
        } else return -1;
		...
		log_info("FEP Server IP = %s", ins->fepserver_ip);
		log_info("FEP Server Port = %s", ins->fepserver_port);
