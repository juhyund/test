...
void udpProxyComm(int s, const char* ip, const char* port, char* buf, int buf_len, char** rdbuf, int *rbuf_len) 
{
	int slen;
	...
	int sentSize = 0;
	const char eof = 0x1A;
	char rbuf[PTLS_SSL_MAX_CONTENT_LEN];
...
{
	close(s);
	log_info("Sockt has been Closed. [Socket Discriptor = %d]", s);
