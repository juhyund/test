...
#ifndef PTLS_NIST_KW_H
#define PTLS_NIST_KW_H
...
} ptls_nist_kw_context;

#else  /* PTLS_NIST_key wrapping_ALT */
#include "nist_kw_alt.h"
#endif /* PTLS_NIST_KW_ALT */

/**
 * \brief           This function initializes the specified key wrapping context
 *                  to make references valid and prepare the context
 *                  for ptls_nist_kw_setkey() or ptls_nist_kw_free().
 *
 * \param ctx       The key wrapping context to initialize.
 *
 */
void ptls_nist_kw_init( ptls_nist_kw_context *ctx );
 ...
 * \return          \c PTLS_ERR_CIPHER_BAD_INPUT_DATA for any invalid input.
 * \return          \c PTLS_ERR_CIPHER_FEATURE_UNAVAILABLE for 128-bit block ciphers
 *                  which are not supported.
 * \return          cipher-specific error code on failure of the underlying cipher.
 */
int ptls_nist_kw_setkey( ptls_nist_kw_context *ctx,
 ...
 */
void ptls_nist_kw_free( ptls_nist_kw_context *ctx );

/**
 * \brief           This function encrypts a buffer using key wrapping.
 *
 * \param ctx       The key wrapping context to use for encryption.
 * \param mode      The key wrapping mode to use (PTLS_KW_MODE_KW or PTLS_KW_MODE_KWP)
 ...
 *
 * \return          \c 0 on success.
 * \return          \c PTLS_ERR_CIPHER_BAD_INPUT_DATA for invalid input length.
 * \return          cipher-specific error code on failure of the underlying cipher.
 */
int ptls_nist_kw_wrap( ptls_nist_kw_context *ctx, ptls_nist_kw_mode_t mode,
 ...
 * \return          \c 0 on success.
 * \return          \c PTLS_ERR_CIPHER_BAD_INPUT_DATA for invalid input length.
 * \return          \c PTLS_ERR_CIPHER_AUTH_FAILED for verification failure of the ciphertext.
 * \return          cipher-specific error code on failure of the underlying cipher.
 */
int ptls_nist_kw_unwrap( ptls_nist_kw_context *ctx, ptls_nist_kw_mode_t mode,
                            const unsigned char *input, size_t in_len,
                            unsigned char *output, size_t* out_len, size_t out_size);


#if defined(PTLS_SELF_TEST) && defined(PTLS_AES_C)
/**
 ...
 */
int ptls_nist_kw_self_test( int verbose );
#endif /* PTLS_SELF_TEST && PTLS_AES_C */

#ifdef __cplusplus
}
#endif

#endif /* PTLS_NIST_KW_H */
