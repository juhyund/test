...
tls_session_t *tls_new_session(TALLOC_CTX *ctx, fr_tls_server_conf_t *conf, REQUEST *request, bool client_cert)
{
	...
	DEBUG(" Finding Where %s, %d " ,__FUNCTION__, __LINE__ );
...
int cbtls_verify(int ok, X509_STORE_CTX *ctx)
{
	...
	vp_cursor_t cursor;
	X509		*client_cert;
	VALUE_PAIR  *vpp; // VP for client's packet
#if OPENSSL_VERSION_NUMBER >= 0x10100000L && !defined(LIBRESSL_VERSION_NUMBER)
	...
	if (certs && identity && (lookup <= 1) && common_name[0] && subject[0]) {
		...
		RDEBUG(" CN NAME IS = %s , lookup %d ", vp->vp_octets, lookup );
		vp_prints_value(CNName, 255, vp, 0 );

		/* if the it is TLS-Cliennt-Cert-Common Name, 
		* Compare to User-Name of request packet
		* lookup [0] Client [1] Server - added by hyun5
		*/
		if( lookup ==  0 ) {
			for (vpp = fr_cursor_init(&cursor, &vpp); vpp; vpp = fr_cursor_next(&cursor)) {
			     VERIFY_VP(vpp);
    			 if ((vpp->da->vendor == 0) && ((vpp->da->attr & 0xFFFF) > 0xff)) continue;
		     	 if ( strcmp(vpp->da->name, "User-Name") == 0 ){
        		  vp_prints_value(UserName, 255, vpp, 0 );
	    		  }
 		  	}	

			RDEBUG(" CN & User-Name Cmpare........ %s <-> %s", CNName, UserName );

			// If the two factors don't match, the certification fails. - added by hyun5
			if( strcmp( strcat(CNName, "@tls.com"), UserName ) != 0 ){
				RERROR("User-Name and CN are different!");
				//err = X509_V_ERR_UNKNOWN_DEVICE_NAME;
				my_ok = 0;
			}
...
post_ca:
	...
	DEBUG("cipher suite(config file): %s , %s, %s, %d", conf->cipher_list, __FILE__, __FUNCTION__, __LINE__);

	/* Print Ciphers added by hyun5*/
	ssl = SSL_new(ctx);	

	for (int i = 0;; i++) {
		p = SSL_get_cipher_list(ssl, i);
		if (p == NULL)	break;
	    if (i != 0)	DEBUG( " cipher list [%d] %s \n",i, p);
	}
	...
	for (int i = 0;; i++) {
		p = SSL_get_cipher_list(ssl, i);
		if (p == NULL)	break;
		if (i != 0)	DEBUG( "using ciphers [%d] %s \n", i, p);
	}
