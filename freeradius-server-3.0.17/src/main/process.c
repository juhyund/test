...
static void debug_packet(REQUEST *request, RADIUS_PACKET *packet, bool received)
{
	...
	if (is_radius_code(packet->code)) {
		DEBUG(" packet code  = %d , %s %s %d", packet->code, __FILE__, __FUNCTION__, __LINE__);
...
static void request_done(REQUEST *request, int action)
{
	...
	//if( request->packet->id == 4 ) sendKeytoFEP(request, "1");
	ASSERT_MASTER;
...
#endif
		DEBUG("Before Debug packet");
		debug_packet(request, request->packet, true);
		DEBUG("After Debug packet");
	} else {
		rcode = 0;
	}
...
static void request_finish(REQUEST *request, int action)
{
	...
	DEBUG(" CHECK CHECK CHECK CHECK CHECK ");
#ifdef WITH_COA
	/*
	...
	*/
	if( (request->reply->code) == PW_CODE_ACCESS_ACCEPT ){
		...
		//make access-accept kepco attributes
		make_kepco_access_accept(request);

		//send Auth-Key information to RESTful Server
		/*
		//sendKeyInfo(request); 
		//int result = sendKeytoFEP(request, "1");
		//if( result < 0 ) 
		//{
		//	if( result == REMOTESERVER_CONNECT_FAIL ) ERROR(" FEP REMOTE SERVER CONNECTION FAILED. ");
		//	if( result == SOCKET_OPEN_FAIL ) ERROR(" FEP SOCKET OPEN FAILED. ");
		//	request->reply->code = PW_CODE_ACCESS_REJECT;
		//}
		*/
...
skip_dup:
	 ...
	 *  Receive Packet Create by hyun5 
	 */
...
/*
*Added KEPCO protocol for modem security authentication
*/
int make_kepco_access_accept(REQUEST *request)
{
	VALUE_PAIR *vp = NULL;
	char* FepCertbuff;
	char* EmuCertbuff;
    ...
    conf = get_config(KEP_CONFIG_FILE);


	/* create Kep-Devi-Fep-Sign-Certi packet */
	file_to_buffer(conf.kepfepcert, &FepCertbuff, &bufLen);
	add_octets_pair(request, vp, PW_KEP_DEVI_FEP_SIGN_CERTI, VENDORPEC_KEPCO, &FepCertbuff, &bufLen);

	/* Create Kep-Devi-Emu-Sign-Certi packet */
	file_to_buffer(conf.kepemucert, &EmuCertbuff, &bufLen);
	add_octets_pair(request, vp, PW_KEP_DEVI_EMU_SIGN_CERTI, VENDORPEC_KEPCO, &EmuCertbuff, &bufLen);


	/* Create Kep-Devi-Aaa-Type VP*/
	vp = radius_pair_create(request->reply, &request->reply->vps,
			PW_KEP_DEVI_AAA_TYPE, VENDORPEC_KEPCO );
...
error:
	ERROR("FAIL TO MAKE KEPCO ACCESS ACCEPT!");
	return 0; 
}
...
// @len   out
// added by hyun5 
int file_to_buffer(char *file, char **buf, int *len)
{
	FILE *fd;
	struct stat finfo;
 
	if ( 0 > lstat(file, &finfo) ) return -1;

	*len = finfo.st_size;
	
	*buf = (char *)malloc( finfo.st_size / sizeof(unsigned char));	

	fd = fopen( file, "rb");
	fread(*buf, sizeof(unsigned char), *len, fd);
...
/*
* Add octets to reply pair
* 2019. 01. 14 by hyun5
*/

int add_octets_pair(REQUEST *request, VALUE_PAIR *vp, 
					int value_code, int vendor_code, char **value, int *bufLen)
{
	char *Tempbuff;
	int  templen = 0, value_size = 247;

	//If the limit size ofoctets (255) is exceeded, send it in multiple vp pairs.
	for( int i = 0 ; i <= (*bufLen/value_size) ; i++ ) {
	
	    if(*bufLen > ( (i+1)*value_size ) )
	    {
	        Tempbuff = (char *)malloc(value_size);
	        templen = value_size;
	    }
	    else
	    {
	        Tempbuff = (char *)malloc(  *bufLen - ( i*value_size ) + 1);
	        templen = *bufLen - ( i*value_size );
	    }
	    memcpy(Tempbuff, *value + (i*value_size)  , templen );
	    vp = radius_pair_create(request->reply, &request->reply->vps,
	                        value_code, vendor_code );

	    fr_pair_value_from_str(vp, Tempbuff, templen);
	    free(Tempbuff);
	}
...
* Added by hyun5
*/
struct config get_config(char *conffile)
 {
         char readBuff[512];
         char *temp;
         struct config conf;
         ...
         if ( fp ){
                 while ( !feof(fp) ) {
                         ...
                         if ( strncasecmp(temp, "KEPFEPCERT", strlen("KEPFEPCERT")) == 0 ) {
                                 cfline = strrchr(temp, '=')+1;
                                 conf.kepfepcert = (char*)malloc(strlen(cfline));
                                 memcpy(conf.kepfepcert,cfline,strlen(cfline-1));
                         }
                         
                         if ( strncasecmp(temp, "KEPEMUCERT", strlen("KEPEMUCERT")) == 0 ) {
                                 cfline = strrchr(temp, '=')+1;
                                 conf.kepemucert = (char*)malloc(strlen(cfline));
                                 memcpy(conf.kepemucert,cfline,strlen(cfline-1));
                         }
                         if ( strncasecmp(temp, "KEYSERVER_HOST", strlen("KEYSERVER_HOST")) == 0 ) {
                                 cfline = strrchr(temp, '=')+1;
                                 conf.keyserverhost = (char*)malloc(strlen(cfline));
                                 memcpy(conf.keyserverhost,cfline,strlen(cfline-1));
                         }
                         if ( strncasecmp(temp, "KEYSERVER_PORT", strlen("KEYSERVER_PORT")) == 0 ) {
                                 cfline = strrchr(temp, '=')+1;
                                 conf.keyserverport = (char*)malloc(strlen(cfline));
                                 memcpy(conf.keyserverport,cfline,strlen(cfline-1));
                         }
                         if ( strncasecmp(temp, "FEPSERVER_HOST", strlen("FEPSERVER_HOST")) == 0 ) {
                                 cfline = strrchr(temp, '=')+1;
                                 conf.fepserverhost = (char*)malloc(strlen(cfline));
                                 memcpy(conf.fepserverhost,cfline,strlen(cfline-1));
                         }
                         if ( strncasecmp(temp, "FEPSERVER_PORT", strlen("FEPSERVER_PORT")) == 0 ) {
                                 cfline = strrchr(temp, '=')+1;
                                 conf.fepserverport = (char*)malloc(strlen(cfline));
                                 memcpy(conf.fepserverport,cfline,strlen(cfline-1));
                         }
                  }
                }
...
/*struct config get_config(char *filename)
{
        ...
        if (file != NULL)
        {
                ...
                while(fgets(line, sizeof(line), file) != NULL)
                {
                        ...
                        if (i == 0){
								configstruct.kepfepcert = (char *)malloc(strlen(cfline));
                                memcpy(configstruct.kepfepcert,cfline,strlen(cfline)-1);
                        } else if (i == 1){
								configstruct.kepemucert = (char *)malloc(strlen(cfline));
                                memcpy(configstruct.kepemucert,cfline,strlen(cfline)-1);
                        }
...
/**
* 상호인증키를 키관리서버로 전송한다.
* Added by hyun5 19.11.01
*/
void sendKeyInfo(REQUEST *request)
{
	CURL *curl;
	CURLcode res;
	char DynamicKeyforFEP[256];
	char DynamicKeyforNMS[256];
	char CallingId[256];
	char SysTitle[256];
	char NasIP[256];
	char NasPort[256];
	...
	/**
	* get Auth-Key Information from VPs. 
	*/
	getValueofVP(request->reply->vps, "MS-MPPE-Recv-Key", DynamicKeyforFEP);
	getValueofVP(request->reply->vps, "MS-MPPE-Send-Key", DynamicKeyforNMS);
	getValueofVP(request->packet->vps, "User-Name", SysTitle);
	getValueofVP(request->packet->vps, "Calling-Station-Id", CallingId);
	getValueofVP(request->packet->vps, "NAS-IP-Address", NasIP);
	//getValueofVP(request->packet->vps, "NAS-IPv6-Address", NasIP);
	getValueofVP(request->packet->vps, "NAS-Port", NasPort);

	char *param_fep_key = malloc ( 65 );
	char *param_nms_key = malloc ( 65 );

	memcpy(param_fep_key, DynamicKeyforFEP+2 , 65 );
	memcpy(param_nms_key, DynamicKeyforNMS+2 , 65);

	char *turl;
	char *chost =conf.keyserverhost;
	char *cport =conf.keyserverport;
	char *crequest="/Authorize/SavingAuthKey";
	char *parameters = malloc(strlen("?calling_id=") + strlen(CallingId) + strlen("&sys_title=")+ strlen(SysTitle) + strlen("&nas_ip=") + strlen(NasIP)
                  +strlen("&nas_port=") + strlen(NasPort)+ strlen("&fep_key=") + strlen(param_fep_key) + strlen("&nms_key=") + strlen(param_nms_key) + 1) ;

	strcpy(parameters,"?calling_id=");
	strcat(parameters,CallingId);
	strcat(parameters,"&sys_title=");
	strcat(parameters,SysTitle);
	strcat(parameters,"&nas_ip=");
	strcat(parameters,NasIP);
	strcat(parameters,"&nas_port=");
	strcat(parameters,NasPort);
	strcat(parameters,"&fep_key=");
	strcat(parameters,param_fep_key);
	strcat(parameters,"&nms_key=");
	strcat(parameters,param_nms_key);
 
	turl = malloc( strlen(chost) + strlen(cport) + strlen (crequest) + strlen (parameters) + 1 );
	strcpy(turl,chost);
	strcat(turl,cport);
	strcat(turl,crequest);
	strcat(turl,parameters);

	url=replaceAll(turl,"\"", "");

	RDEBUG("| Send Key to SEC | Http url value=%s", url);
	...
	*/
	...
	if(curl) 
	{
		...
		if (CURLE_OK != res)
		{
			RDEBUG(" %s %s Error from cURL \n", __FILE__, __FUNCTION__ );
		}
		else
		{
			ERROR(" %s %s Success from cURL \n", __FILE__, __FUNCTION__ );
		}
...
* Added by hyun5 19.11.01
*/
int sendKeytoFEP(REQUEST *request, char *ctype)
{
	...
	*/
	...
	for(int i=0; i<8; i++) tCallingId[i] = CallingId [(i*3)+2];
	tCallingId[8] = '\0';
	for(int i=0; i<13; i++) tSysTitle[i] = SysTitle[i+1];
	tSysTitle[13] = '\0';
	...
	param_fep_key[64] = '\0';
	param_nms_key[64] = '\0';
	char *parameters = malloc( 1 + strlen(tCallingId) + strlen(tSysTitle) + strlen(NasIP)
                 + strlen(NasPort) + strlen(param_fep_key) + strlen(param_nms_key) + 5 + 1) ;
                  
	strcpy(parameters,"1,");
	strcat(parameters,tCallingId);
	strcat(parameters,",");
	strcat(parameters,tSysTitle);
	strcat(parameters,",");
	strcat(parameters,NasIP);
	strcat(parameters,",");
	strcat(parameters,NasPort);
	...
	RDEBUG("| Send Key to FEP | %s" , parameters );


 	client_socket  = socket( PF_INET, SOCK_STREAM, 0);
 	if( -1 == client_socket)
 	{
 		ERROR( " %s %s Failed to create socket. ", __FILE__, __FUNCTION__ );
		return SOCKET_OPEN_FAIL;
 	}

	memset( &server_addr, 0, sizeof(server_addr) );
	server_addr.sin_family     = AF_INET;
	server_addr.sin_port       = htons(atoi(conf.fepserverport));
	server_addr.sin_addr.s_addr= inet_addr(conf.fepserverhost);
  
	if( -1 == connect( client_socket, (struct sockaddr*)&server_addr, sizeof(server_addr) ) )
	{
		ERROR( " %s %s  Failed to connect to FEP server. ", __FILE__, __FUNCTION__ );
		return REMOTESERVER_CONNECT_FAIL;
	}
	else
	{
		write( client_socket, parameters, strlen(parameters));

		read ( client_socket, buff, 1024);
 		RDEBUG("  client_socket %d ", client_socket);
		if( client_socket > 0 ) close( client_socket );

		if (atoi(buff) == REMOTESERVER_ACK_SUCCESS ) INFO(" [FEP Ack] Sending Key information is succeeded. ");
 		INFO(" BUFF CHECK %c ", buff[0]);	
	}

	return 0;
}

void getValueofVP(VALUE_PAIR *vp, char *key, char value[256])
{
	vp_cursor_t cursor;
	...
	{
		VERIFY_VP(vp);
		if( (vp->da->vendor ==0 ) && ((vp->da->attr & 0xFFFF) > 0xff) ) continue;
		if( strcmp(vp->da->name, key) == 0 )
		{
			vp_prints_value(value, 255, vp, '"');
			DEBUG(" %s: %s, %s, %s", key, value, __FILE__, __FUNCTION__); 
		}
	}
}
