...
int rad_authenticate(REQUEST *request)
{
	 ...
	 */
	DEBUG(" In rad_authenticate 0 : %s , %s, %d", __FILE__, __func__, __LINE__);
	if (request->proxy) {
		int code = 0;
...
authenticate:
#endif
	/*
	 *	Validate the user Add the vps (Reply) - comment by hyun5
	 */
	do {
	...
	DEBUG(" After rad_check_password %s %s, %d", __FILE__, __FUNCTION__, __LINE__);
