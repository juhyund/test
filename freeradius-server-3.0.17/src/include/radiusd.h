...
/*
* Constants and Struct required to read the protocol
* Added by hyun5
*/
#define KEP_CONFIG_FILE "/root/freeradius-server-3.0.17/config.conf" //property file
#define DELIM "="

struct config
{
   char *kepfepcert; //!< Kep-Devi-Fep-Sign-Certi file
   char *kepemucert; //!< Kep-Devi-Emu-Sign-Certi file
   char *keyserverhost; //!< Key Management Server host
   char *keyserverport; //!< Key Management Server port
   char *fepserverhost; //!< fep Server host
   char *fepserverport; //!< fep Server port
} config;
...
/*SendAuth-Key information to Restful Server using cURL*/
void sendKeyInfo(REQUEST *request);
/*SendAuth-Key information to FEP Server using cURL*/
int sendKeytoFEP(REQUEST *request, char *ctype);
/*get Value of VALUE_PAIR*/
void getValueofVP(VALUE_PAIR *vp, char *key, char value[256]);

/*define by hyun5*/
#define REMOTESERVER_ACK_SUCCESS	0
#define REMOTESERVER_CONNECT_FAIL	-1
...
int add_octets_pair(REQUEST *request, VALUE_PAIR *vp, int value_code, int vendor_code, char **value, int *bufLen);
